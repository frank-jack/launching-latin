...
struct ColorsMatching: View {
    ...
    var body: some View {
        ZStack {
            ...
            if begun {
                VStack {
                    if (matchesCorrect+matchesWrong) < ModelData().colors.count {
                        Button("Submit Match") {
                                ...
                                VStack {
                                    ForEach(orderLatin, id: \.self) { i in
                                        ZStack {
                                            ...
                                            Button(ModelData().colors[orderLatin[i]].latin) {
                                    ...
                                    ForEach(orderEnglish, id: \.self) { i in
                                        ZStack {
                                            ...
                                            Image("paper")
                                                .resizable()
                                                .frame(width: UIScreen.main.bounds.width*3/4.14, height: UIScreen.main.bounds.height*2/8.96)
                                            Button(ModelData().colors[orderEnglish[i]].english) {
                                                englishChoice = orderEnglish[i]
                                                imageNumberEnglish = i
                                            }
                                            .foregroundColor(trueColors[ModelData().colors[orderEnglish[i]].english])
                                        }
                                    }
                                }
                            }
                        ...
                        VStack {
                            Text("Your Score was "+String(matchesCorrect)+"/"+String(ModelData().colors.count))
...
struct ColorsMatching_Previews: PreviewProvider {
