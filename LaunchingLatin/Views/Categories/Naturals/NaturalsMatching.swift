...
struct NaturalsMatching: View {
    ...
    var body: some View {
        ZStack {
            ...
            if begun {
                VStack {
                    if (matchesCorrect+matchesWrong) < ModelData().naturals.count {
                        Button("Submit Match") {
                                ...
                                VStack {
                                    ForEach(orderLatin, id: \.self) { i in
                                        ZStack {
                                            ...
                                            Button(ModelData().naturals[orderLatin[i]].latin) {
                                    ...
                                    ForEach(orderEnglish, id: \.self) { i in
                                        ZStack {
                                            ...
                                            Image("paper")
                                                .resizable()
                                                .frame(width: UIScreen.main.bounds.width*3/4.14, height: UIScreen.main.bounds.height*2/8.96)
                                            Button(ModelData().naturals[orderEnglish[i]].english) {
                        ...
                        VStack {
                            Text("Your Score was "+String(matchesCorrect)+"/"+String(ModelData().naturals.count))
                ...
                Button("Begin Matching!") {
                    while orderLatin.count<ModelData().naturals.count {
                        temp=Int.random(in: 0...ModelData().naturals.count-1)
                        if !orderLatin.contains(temp) {
                            orderLatin.append(temp)
                        }
                    }
                    while orderEnglish.count<ModelData().naturals.count {
                        temp=Int.random(in: 0...ModelData().naturals.count-1)
...
struct NaturalsMatching_Previews: PreviewProvider {
