...
struct ShapesQuiz: View {
    ...
    var body: some View {
        ZStack {
            ...
            if finished {
                VStack {
                    Text("Your Score was "+String(questionsRight)+"/"+String(ModelData().shapes.count))
                    ...
                    if !answered {
                            ...
                            VStack {
                                ZStack {
                                    ...
                                    VStack {
                                        Text("What is the\nEnglish for")
                                            .font(.custom("Palatino-Roman", fixedSize: 34).weight(.black))
                                        Text(ModelData().shapes[temp].latin+"?")
                                            .font(.custom("Palatino-Roman", fixedSize: 34).weight(.black))
                                    }
                                }
                                TextField("Answer", text: $answer)
                                    ...
                                    .padding(.horizontal)
                                Button("Submit") {
                                    if (answer.caseInsensitiveCompare(ModelData().shapes[temp].english) == .orderedSame) {
                                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                                            answered = true
                                        }
                                        correct = true
                            ...
                            VStack {
                                ZStack {
                                    ...
                                    VStack {
                                        Text("What is the\nLatin for")
                                            .font(.custom("Palatino-Roman", fixedSize: 34).weight(.black))
                                        Text(ModelData().shapes[temp].english+"?")
                                            .font(.custom("Palatino-Roman", fixedSize: 34).weight(.black))
                                    }
                                }
                                TextField("Answer", text: $answer)
                                    ...
                                    .padding(.horizontal)
                                Button("Submit") {
                                    if (answer.caseInsensitiveCompare(ModelData().shapes[temp].latin) == .orderedSame) {
                                        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
                                            answered = true
                                        }
                                        correct = true
                        ...
                        if correct {
                            VStack {
                                ...
                                if order.count < ModelData().shapes.count {
                                    Button("Next") {
                                        while order.contains(temp) {
                                            temp = Int.random(in: 0...ModelData().shapes.count-1)
                                        }
                                        order.append(temp)
                                        answered = false
                                        answer = ""
                                ...
                                ZStack {
                                    ...
                                    VStack {
                                        Text("That's not\nthe answer...")
                                            .font(.custom("Palatino-Roman", fixedSize: 34).weight(.black))
                                        if latinToEnglish {
                                            Text("The answer was\n"+ModelData().shapes[temp].english+".")
                                                .font(.custom("Palatino-Roman", fixedSize: 34).weight(.black))
                                        } else {
                                            Text("The answer was\n"+ModelData().shapes[temp].latin+".")
                                                .font(.custom("Palatino-Roman", fixedSize: 34).weight(.black))
                                        }
                                    }
                                }
...
struct ShapesQuiz_Previews: PreviewProvider {
